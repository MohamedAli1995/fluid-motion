...
def detector(sendQueue, receiveQueue, _frame, faces, exitFlag, facesNumber):
    ...
    try:
        _frame, currentFaces, exitFlag, currentFacesNumber = detectSingleFace(_frame, faces, exitFlag, facesNumber,
                                                                              sendQueue)
    except:
    ...
    while True:
        while (receiveQueue.empty() == False):
            ...
            _frame, currentFaces, exitFlag, currentFacesNumber = detectSingleFace(_frame, currentFaces, exitFlag,
                                                                                  currentFacesNumber, sendQueue)
            if exitFlag == 1:
                break
        if exitFlag == 1:
            break
...
def tracker(receiveQueue, sendQueue, mainQueue, _frame, faces, exitFlag, facesNumber):
    ...
    while True:
        while (receiveQueue.empty() == False):
            ...
            if exitFlag == 1:
                break
            currentFrame, faces, exitFlag, facesNumber, boxes = trackFaces(currentFrame, faces, exitFlag, facesNumber,
                                                                           boxes, receiveQueue, sendQueue, mainQueue)
        if exitFlag == 1:
            break
        currentFrame, faces, exitFlag, facesNumber, boxes = trackFaces(currentFrame, faces, exitFlag, facesNumber,
                                                                       boxes, receiveQueue, sendQueue, mainQueue)
...
def trackFaces(currentFrame, faces, exitFlag, facesNumber, boxes, receiveQueue, sendQueue, mainQueue):
    boxesA = np.copy(boxes)
    while (facesNumber != 0 and exitFlag == 0):
        # Create tracker
        tracker = cv2.MultiTracker_create()
        for face in faces:
            ok = tracker.add(cv2.TrackerKCF_create(), currentFrame,
